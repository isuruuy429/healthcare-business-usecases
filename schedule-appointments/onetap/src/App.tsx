...
function App() {
  ...
  const handleAppointment = async () => {
    if (selectedDoctor && appointmentDate) {
      ...
      const url = `/fhir8082/r4/Slot?startDate=${startDate}&endDate=${endDate}&practitioner=${practitionerId}`;
  ...
  const fetchLocationNames = async (slots: Slot[]) => {
    ...
    for (const slot of slots) {
      if (!newLocations[slot.locationReference]) {
        try {
          const locationUrl = `/fhir8084/r4/Location/${slot.locationReference}`;
  ...
  useEffect(() => {
    if (appointmentCreated && selectedDoctor && selectedSlot) {
      ...
      const timer = setTimeout(() => {
        navigate('/appointment-receipt', {
          state: {
            doctorName: selectedDoctor.name,
            patientName: 'Agatha Cristy',
  ...
  const createAppointment = async () => {
    ...
    try {
      const appointmentUrl = `/fhir8083/r4/Appointment`;
